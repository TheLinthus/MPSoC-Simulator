...
QStringList ApplicationController::getRunningApplicationsList() const {
    ...
    for (Application *app : runningList) {
        QString displayName = QString("%1: %2").arg(app->getUid()).arg(app->getName());
        if (!app->isAlive())
            displayName.append(tr("[dead]"));
        list << displayName;
    }
...
Application *ApplicationController::run(QString name) {
    for (ApplicationGroup *group : applicationsGroupList) {
        if (group->isEnabled()) {
            ...
            if (app != 0) {
                app = app->clone();
                app->setColor(defaultColors[appUIDSequence % COLORS_COUNT]);
...
void ApplicationController::kill(int index) {
    auto app = runningList.value(index);
    app->kill();
